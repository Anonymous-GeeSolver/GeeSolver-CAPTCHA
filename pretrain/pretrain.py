...
def get_args_parser():
    ...
    parser.add_argument('--mask_radio', type=float, default=0.3, help='mask radio for reconstruction')
...
if __name__ == '__main__':
    ...
    opt.exp_name = f'{opt.dataset_name}-{opt.label_file}-{opt.unlabeled_number}-{opt.num_layer}-{opt.mask_radio}-{opt.manualSeed}'
    ...
    while True:
        ...
        with torch.cuda.amp.autocast():
            loss, _, _ = model(images, mask_ratio=opt.mask_radio)
        ...
        if iteration % opt.showInterval == 0 or iteration == 0:
            ...
            with torch.no_grad():
                ...
                loss, y, mask = model(images_test, mask_ratio=opt.mask_radio)
